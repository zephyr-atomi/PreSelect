...
def _load_checkpoint(queue, args):
    ...
    if args.load_dir.endswith('/'):
        ...
        args.overwrite_iteration = None
        
    if args.load_dir.split('/')[-1].startswith('iter_'):
        iter_str = args.load_dir.split('/')[-1]
        iteration = int(iter_str.split('_')[-1])
        args.overwrite_iteration = str(iteration)
    ...
    if args.overwrite_iteration is not None:
        sys.argv.append('--overwrite-iteration')
        sys.argv.append(args.overwrite_iteration)
    ...
    for vp_rank in range(vp_size):
        ...
        for pp_rank in range(pp_size):
            ...
            for layer_num in range(len(models[0].decoder.layers)):
                ...
                if md.linear_bias:
                    message["dense bias"] = layer.self_attention.linear_qkv.bias.data
    ...
    if md.model_type == 'BERT':
        # TODO
        raise ValueError(f"Unsupported model type: {md.model_type}")
        message = {
